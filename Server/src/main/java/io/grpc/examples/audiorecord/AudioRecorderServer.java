...
package io.grpc.examples.audiorecord;
...
public class AudioRecorderServer {
  private static final Logger logger =
      Logger.getLogger(AudioRecorderServer.class.getName());

  public static void main(String[] args) throws InterruptedException, IOException {
    // Service class implementation
    AudioTranscribeGrpc.AudioTranscribeImplBase svc = new AudioTranscribeGrpc.AudioTranscribeImplBase() {
      @Override
      public StreamObserver<AudioBlob> speechToText(final StreamObserver<TranscribedText> responseObserver) {
        // Set up manual flow control for the request stream. It feels backwards to configure the request
        // stream's flow control using the response stream's observer, but this is the way it is.
        final ServerCallStreamObserver<TranscribedText> serverCallStreamObserver =
            (ServerCallStreamObserver<TranscribedText>) responseObserver;
        ...
        return new StreamObserver<AudioBlob>() {
          @Override
          public void onNext(AudioBlob request) {
            // Process the request and send a response or an error.
            try {
              // Accept and enqueue the request.
              //String name = request.getName();
	      ByteString val = request.getChunk();
              ...
              logger.info("<-- " + message);
              TranscribedText reply = TranscribedText.newBuilder().setText(message).setFinal(false).build();
	    ...
	    String message = "Data completely received";
            logger.info("<-- " + message);
	    boolean done = true;
            TranscribedText reply = TranscribedText.newBuilder().setText(message).setFinal(done).build();
            responseObserver.onNext(reply);
